...
if($dbok){
	/*
	$query = "select `ADDRESS` from `users` order by `ID` desc limit 1";
	if($res = $my->query($query)){
		if($r = $res->fetch_assoc()){
			$address = trim($r['ADDRESS']);
			$address = explode(',',$address);
			if(count($address)==2){
				$selLon = trim($address[0]);
				$selLat = trim($address[1]);
			}
		}
	}
	*/
...
// 			var point = {lon:'59.616772',lat:'36.260468'};
			var point = {lon:'<?php echo $selLon; ?>',lat:'<?php echo $selLat; ?>'};
...
//         if(points.length==0){
          var position = new OpenLayers.LonLat(point.lon,point.lat).transform( fromProjection, toProjection);
//         }else{
...
// 				map.addLayer(newl);
				...
				var markers = new OpenLayers.Layer.Vector("Markers");// , {eventListeners: markerListeners});
        map.addLayer(markers)
				
				var marker = new OpenLayers.Feature.Vector(
					new OpenLayers.Geometry.Point( 13.524110356890848,52.45536065004745 ).transform(fromProjection, toProjection),
					{description:'<a href="reports/p1.jpg" target="_blank"><img style="width:250px" src="reports/p1.jpg" /></a><br/>Disroyed Pedestrian'} ,
					{externalGraphic: 'img/marker.png', graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25  }
        );
				markers.addFeatures(marker);
				
				var marker2 = new OpenLayers.Feature.Vector(
					new OpenLayers.Geometry.Point( 13.386272153348331,52.51657351885607 ).transform(fromProjection, toProjection),
					{description:'<a href="reports/p2.jpg" target="_blank"><img style="width:250px" src="reports/p2.jpg" /></a><br/>Blocked and dirty walking area'} ,
					{externalGraphic: 'img/marker.png', graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25  }
        );
				markers.addFeatures(marker2);

				var marker3 = new OpenLayers.Feature.Vector(
					new OpenLayers.Geometry.Point( 13.525175342988266,52.4558594885045 ).transform(fromProjection, toProjection),
					{description:'<a href="reports/p3.jpg" target="_blank"><img style="width:250px" src="reports/p3.jpg" /></a><br/>Wrong parking on the walking area'} ,
					{externalGraphic: 'img/marker.png', graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25  }
        );
				markers.addFeatures(marker3);

				var marker4 = new OpenLayers.Feature.Vector(
					new OpenLayers.Geometry.Point( 13.520168232687645,52.453646123834545 ).transform(fromProjection, toProjection),
					{description:'<a href="reports/p4.jpg" target="_blank"><img style="width:250px" src="reports/p4.jpg" /></a><br/>Wrong disposing waste beside of street'} ,
					{externalGraphic: 'img/marker.png', graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25  }
        );
				markers.addFeatures(marker4);

				var marker5 = new OpenLayers.Feature.Vector(
					new OpenLayers.Geometry.Point( 13.521816264527072,52.45426105894383 ).transform(fromProjection, toProjection),
					{description:'<a href="reports/p5.jpg" target="_blank"><img style="width:250px" src="reports/p5.jpg" /></a><br/>Wrong disposing waste beside of street'} ,
					{externalGraphic: 'img/marker.png', graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25  }
        );
				markers.addFeatures(marker5);
				...
				var marker = new OpenLayers.Marker(new OpenLayers.LonLat(point.lon,point.lat).transform( fromProjection, toProjection),icon);
...
//           drawLine();
//         }
// 				point = goodToBad(point[0],point[1]);
// 				var spoint = new OpenLayers.Geometry.Point(point.lon,point.lat);
// 				layer.addFeatures([spoint])
