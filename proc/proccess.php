...
function proccessStopsNew($user_id){
  ...
  $sid = 0;
  while($contin){
    $query = "SELECT regtime r,ST_AsText(ushape) p,id FROM `track` `t` WHERE user_id=$user_id and USER_SPEED<=0 and `id` > $sid".(($regdate!='')?" and regtime > '$regdate'":'')." limit 1";
    echo $query."<br/>\n";
    $contin = FALSE;
    if($res = $my->query($query)){
      ...
      if($r = $res->fetch_assoc()){
        ...
        $drgt = $drgt[0];
        $shp = $r['p'];
//         $query = "SELECT max(st_distance(`t`.`ushape`,ST_GeomFromText('$shp'))) mds,id FROM `track` `t` WHERE user_id=$user_id and USER_SPEED<=$walk and `id`>$id and TIME_TO_SEC(TIMEDIFF(`t`.`regtime`,'$rgt'))/60>=20 and date(t.regtime)='$drgt' order by id desc limit 1";
        $query = "SELECT 
        t1.id id1,(t2.id-1) id2,TIME_TO_SEC(TIMEDIFF(`t2`.`regtime`,'$rgt'))/60 tm
        FROM 
        `track` t1 
        left join
        `track` t2 
        ...
        WHERE 
        t1.user_speed <= $walk and 
        t2.user_speed > $walk and 
        t1.user_id = $user_id and  
        t1.id>$id and
        date(t1.regtime)='$drgt' and
        date(t2.regtime)='$drgt'
        limit 1";
        echo $query."<br/>\n";
        $contin = FALSE;
        if($res = $my->query($query)){
          if($r = $res->fetch_assoc()){
            ...
            $sid = $id2;
            $tm = ceil($r['tm']);
            echo "id1 = $id1 , id2 = $id2 , tm = $tm <br/>\n";
            $contin = ($res->num_rows>0);
            if($tm>=$stop_max_time){
              $query = "SELECT max(st_distance(`ushape`,ST_GeomFromText('$shp)')))*1000 mds,max(regtime) rg FROM `track` WHERE id BETWEEN $id1 and $id2 and USER_ID = $user_id";
              echo $query."<br/>\n";
              $contin = FALSE;
              if($res = $my->query($query)){
                if($r = $res->fetch_assoc()){
...
//                     Found STOP
                    echo "STOP $id1 - $id2 | mds = $mds | tm = $tm <br/>\n";
                    $tmp = addStop($user_id,$r['rg'],$shp,$id2);
                    if($tmp!==FALSE){
                      $stops[] = $tmp;
                    }
                  }
...
function proccessData(){
  ...
  if($ures=$my->query($query)){
    while($ur=$ures->fetch_assoc()){
      ...
      while($contin){
        $first_id = 0;
        $query = "SELECT t2.id FROM `track` t1 left join `track` t2 on (t1.id<t2.id and t1.user_id=t2.user_id and t1.travel_mode=t2.travel_mode) WHERE t1.user_speed <= 0 and t2.user_speed>0 and t1.user_id = $user_id and t1.travel_mode = 0 and t1.id>$last_id limit 1";
        echo $query."<br/>\n";
        $contin = FALSE;
        if($res = $my->query($query)){
          if($r = $res->fetch_assoc()){
            $contin = TRUE;
            $first_id = $r['id'];
            $query = "update `track` set `travel_mode` = $travel_mode where `id`<$first_id and `travel_mode`=0 and `user_speed` <= 0";
            $my->query($query);
            echo "fase = 2 , query = $query <hr/>";          
          }
        }
        $last_id = 0;
        $query = "SELECT id FROM `track`  WHERE user_speed <= 0 and user_id = $user_id and travel_mode = 0 and id>$first_id limit 1";
        echo $query."<br/>\n";
        $contin = FALSE;
        if($res = $my->query($query)){
          if($r = $res->fetch_assoc()){
        ...
        $contin = FALSE;
        if($last_id>$first_id && $first_id>0){
